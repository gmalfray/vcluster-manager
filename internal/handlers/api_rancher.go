...
// rancherCleanupManifest is the official rancher-cleanup job manifest. It's
// no longer used by the pair/unpair handlers above (that logic moved to
// service.UnpairRancher, which keeps its own copy) but vcluster.go's delete
// flow still deploys it directly when auto-unpairing ahead of a deletion.
