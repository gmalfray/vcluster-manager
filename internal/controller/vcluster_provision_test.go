...
func TestProvisioningAppliesTheNamespaceAndTheSubstitutions(t *testing.T) {
	...
	r := &VClusterReconciler{Client: k8sClient, Ops: ops, Cell: "preprod",
	...
	requireVCCond(t, got, v1alpha1.CondResourcesProvisioned, metav1.ConditionTrue, "Applied")
}
...
func TestProvisioningIsIdempotent(t *testing.T) {
	...
	for i := range 3 {
		fresh := &VClusterReconciler{Client: k8sClient, Ops: newFakeProvisioner(), Cell: "preprod"}
...
func TestProvisioningOnlyOwnsWhatItDeclares(t *testing.T) {
	ctx := context.Background()
	r := &VClusterReconciler{Client: k8sClient, Ops: newFakeProvisioner(), Cell: "preprod"}
