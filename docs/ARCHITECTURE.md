# Architecture — vCluster Manager

Description technique détaillée de l'application. Pour les règles à destination
des assistants IA, voir [`AGENTS.md`](../AGENTS.md). Pour la portabilité, voir
`FORK.md`.
...
- **ArgoCD version management** : deux niveaux de gestion
  ...
  - Le meme fichier de base (`lib/tenant-template/argocd/base/configmap-argocd-cm.yaml`) porte un `resource.exclusions` qui exclut le kind `Pod` du cache du controller ArgoCD — contournement d'un panic upstream (`argoproj/argo-cd#26529`, nil map dans `populatePodInfo` pour un pod sans resources dans un vcluster avec LimitRange), toujours ouvert au 2026-08-08 (aucune release, y compris la 3.5.0, ne l'a corrige). A retirer quand corrige upstream, voir `TODO.md` §« A retirer quand resolu ». Ce fichier vit dans fluxprod, pas dans ce repo.
...
### Vault webhook (tenant)
- Le generator ne produit que l'overlay par vcluster (`tenant/vault-webhook_kustomization.yaml.tmpl` + `tenant/vault-webhook/kustomization.yaml.tmpl`) : un patch JSON6902 qui remplace `metadata.namespace` (`flux-system` -> `vcluster-{name}`) et ajoute `spec.kubeConfig.secretRef` sur le HelmRelease de base
- La base elle-meme (`lib/tenant-template/vault-webhook/vault_helmrelease.yaml`, source du chart, `chartRef`/`sourceRef`, `install.createNamespace`) vit uniquement dans fluxprod — ce repo n'en a jamais eu de copie, ni dans `internal/gitops/templates` ni dans `examples/fluxprod`. Le patch est agnostique de la facon dont la base source son chart (`GitRepository`+`chart:` ou `OCIRepository`+`chartRef`)
- `install.createNamespace: true` sur le HelmRelease cree lui-meme le namespace `vault-system` ; pas besoin d'une Kustomization ou d'un namespace pre-cree cote generateur
- Verifie sur le cluster de recette (2026-08-08, Flux `distribution: flux-2.4.0`, `source-controller: v1.4.1`, `helm-controller: v1.1.0`) : `OCIRepository` n'existe qu'en `v1beta2` sur ce Flux (pas de `v1`), et `HelmRelease.spec.chartRef` (kind `OCIRepository`) fonctionne — le HelmRelease `vault-webhook` reel (`vcluster-demo`) l'utilise et passe `Ready=True`. Aucune contrainte de version Flux (pas besoin de 2.6+) : le bug corrige n'etait qu'un `apiVersion` errone (`v1` au lieu de `v1beta2`)
